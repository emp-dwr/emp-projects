...
# round to nearest 1 mins and calc average
df_phy <- df_phy %>%
...
df_regions <- read_csv('03_Phyto/phycoprobe/supp_files/regions_fluoro.csv')
df_final <- left_join(df_final, df_regions, 'SubRegion')

df_final <- df_final %>%
  extract(geometry, c('Latitude', 'Longitude'), '\\((.*), (.*)\\)', convert = TRUE) %>%
  select(-c(SubRegion, SONDEDEPTH_ft, Date, Month)) %>%
