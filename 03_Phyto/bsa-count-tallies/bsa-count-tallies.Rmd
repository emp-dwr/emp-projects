...
df_phyto_20s <- bind_rows(df_phyto2022, df_phyto2023)
...
df_phyto <- df_phyto %>% mutate(Study = "EMP", .after = StationCode)
...
df_phyto <- df_phyto %>% 
  relocate(GALD, .before = Biovolume1)
...
histogram_plot <- ggplot(df_tally, aes(x = Tally)) +
  ...
  labs(title = "Total Algal Units Counted in EMP Phytoplankton Samples",
       x = "Units Counted",
...
p_barplot +
    labs(title = "Occurrences of < 400 Unit Counts",
       x = "Year",
       y = "Number of Samples",
