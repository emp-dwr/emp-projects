...
fn_fl <- 'Fluoroprobe_GB_Apr23.txt'

# declare filename for water quality data (.csv)
fn_wq <- 'MOPED_Export_60_04172023_GB.csv'
```
...
# read in data
year <- extract_year(fn_fl)
month <- extract_month(fn_wq)
...
# clean up col names
clean_cols <- sapply(colnames(df_fl), function(x) unlist(stringr::str_split(x, "\\.{2,}"))[[1]])
colnames(df_fl) <- paste(clean_cols, first_row, sep='_')
...
keep_cols <- c('Green Algae_ug/l','Bluegreen_ug/l','Diatoms_ug/l','Cryptophyta_ug/l','Green Algae_cells/ml','Bluegreen_cells/ml',
               'Diatoms_cells/ml','Cryptophyta_cells/ml','Total conc._ug/l','Temp. Sample_degC','Date/Time_date')
...
df_fl <- df_fl %>%
  mutate(Year = year(df_fl$DateTime),
         ...
         Time = format(as.POSIXct(df_fl$DateTime), format = "%H:%M:%S"))

# read in wq data
fp_wq <- abs_path(paste0(year,'/',fn_wq), type = 'MOPED')
df_wq <- read_csv(fp_wq, skip = 2)

# clean up, including adding WQ analytes
...
df_wq[is.nan(df_wq)] <- NA

# set run abbv
df_names <- read_csv('03_Phyto/fluoroprobe-data-cleaning/supp_files/run_names.csv')
...
df_final <- df_final %>%
  ...
  subset(select = -c(SubRegion, SONDEDEPTH_ft)) %>%
  relocate(c('DateTime','Date','Time','Month','Year','Latitude','Longitude','Region'))

# remove rows where all fluoroprobe data is 0 (ie. not on)
df_final <- df_final %>%
  filter(`Total conc._ug/l` > 0)

# df_final <- df_final %>%
#    filter_at(vars(c(ends_with('cells/ml'),ends_with('ug/l'))), any_vars(. > 0))
...
fp_exp <- abs_path(paste0('0cat_',fn_exp,'_summary.csv'), type = 'export', year = year)
