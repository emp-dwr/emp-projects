...
df_year_stat <-  merge(df_year_stat, df_grab_vari, by = c('Year','StationCode'), all.x=FALSE, all.y=FALSE)
...
for (station in unique(df_year_stat_top$StationCode)){
  ...
  df_filt <- df_year_stat_top %>%
    ...
    group_by(Name) %>%
    mutate(Percentage = round(sum(CPUETotal_Year)/CPUESum_Year*100,2)) %>%
    ungroup()
  ...
  df_filt$CPUETotal_Year[is.na(df_filt$CPUETotal_Year)] = 0
  ...
  # plot
  plt <- ggplot(df_filt, aes(Year, CPUETotal_Year, group = Name)) +
    geom_line(aes(color = Name), size = 1) +
    geom_point(aes(color = Name), size = 1.7) +
    scale_color_manual(values = getPalette(colorCount)) +
...
# TODO: double check math by summing stations manually
# TODO: START HERE 9/15
...
df_month_stat_top <- df_month_stat %>%
  ...
  mutate(TotalSum = sum(CPUETotal_Month, na.rm = TRUE)) %>%
  arrange(Year, StationCode, desc(TotalSum)) %>%
  ungroup()
...
for (station in unique(df_month_stat_top$StationCode)){
  ...
  df_filt <- df_month_stat_top %>%
    ...
    ungroup %>%
    group_by(Name) %>%
    mutate(Percentage = round(sum(CPUETotal_Month)/CPUESum_Month*100,2)) %>%
    ungroup %>%
  ...
  df_filt$CPUETotal_Month[is.na(df_filt$CPUETotal_Month)] = 0
  df_filt$Percentage[is.na(df_filt$Percentage)] = 0
  ...
  # plot timeseries
  ts <- ggplot(df_filt, aes(Month, CPUETotal_Month, group = Name)) +
    geom_line(aes(color = Name), size = 1) +
    geom_point(aes(color = Name), size = 1.7) +
    scale_color_manual(values = getPalette(colorCount)) +
    ylab(expression(bold('Individuals/m'^2))) +
...
df_report_year_stat <- df_year_stat %>%
  ...
  mutate(Percentage = round(CPUETotal_Year/CPUESum_Year*100, 2)) %>%
  arrange(StationCode, desc(OrgsTotal_Year))
...
df_ry_stat_phy <- df_report_month_stat %>%
  ...
  summarize(OrgsTotal_Month = sum(OrgsTotal_Month), CPUE_Total = sum(CPUETotal_Month)) %>%
  ungroup() %>%
...
# remove extra col
df_ry_stat_phy <- subset(df_ry_stat_phy, select = -c(SumTotal_Month))
...
for (station in unique(df_ry_stat_phy$StationCode)){
  ...
  df_check <- df_filt %>%
    mutate(Total_Check = sum(OrgsTotal_Month)) %>%
    group_by(Phylum) %>%
    mutate(Percent_Check = round(sum(OrgsTotal_Month)/Total_Check*100,2)) %>%
    ungroup()
...
df_phy_percs <- df_phy_percs %>%
  group_by(Phylum) %>%
  summarize(Orgs_Total = sum(OrgsTotal_Month), CPUE_Total = sum(CPUETotal_Month)) %>%
  ungroup() %>%
...
df_phy_stat_percs <- df_phy_stat_percs %>%
  ...
  summarize(Orgs_Total = sum(OrgsTotal_Month), CPUE_Total = sum(CPUE_Total)) %>%
  ungroup() %>%
  group_by(StationCode) %>%
  mutate(Percentage = round(Orgs_Total/sum(Orgs_Total)*100,2)) %>%
