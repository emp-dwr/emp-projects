...
df_month <- df_benthic %>%
  ...
  mutate(GrabCount_Month = length(unique(Grab))) %>%
  ungroup() %>%
  group_by(.dots = c(classif, 'Month', 'Year', 'StationCode', 'GrabCount_Month')) %>%
  summarize(OrgsTotal_Month = sum(Count, na.rm = TRUE), .groups = 'drop') %>%
  mutate(Date = paste(Month, Year)) # TODO: explore adding in day as well

# get grab variable per station per year
df_grab_vari <- df_month %>%
  distinct(Date, Year, StationCode, GrabCount_Month) %>%
  group_by(Year, StationCode) %>%
  summarize(GrabCount_Year = sum(GrabCount_Month), .groups = 'drop') %>%
  group_by(Year) %>%
  mutate(GrabCount_Year = sum(GrabCount_Year)) %>%
  ungroup()

# create df of totals by year per station
df_year <- df_month %>%
  group_by(.dots = c(classif, 'Year','StationCode')) %>%
  summarize(OrgsTotal_Year = sum(OrgsTotal_Month, na.rm = TRUE), .groups = 'drop')

df_year <-  merge(df_year, df_grab_vari, by = c('Year','StationCode'), all.x=FALSE, all.y=FALSE)

# calc CPUE
df_month <- df_month %>%
  group_by(.dots = c(classif, 'Month', 'Year', 'StationCode')) %>% # TODO: redundant, check after confirming right
  mutate(CPUETotal_Month = OrgsTotal_Month/GrabCount_Month/0.052) %>%
  ungroup() %>%
  group_by(.dots = c(classif, 'Month', 'Year')) %>%
  summarize(CPUETotal_Month = sum(CPUETotal_Month), .groups = 'drop')

df_year <- df_year %>%
  group_by(.dots = c(classif, 'Year', 'StationCode')) %>%
  mutate(CPUETotal_Year = OrgsTotal_Year/GrabCount_Year/0.052) %>%
  ungroup() %>%
  group_by(.dots = c(classif, 'Year')) %>%
  mutate(CPUETotal_Year = sum(CPUETotal_Year),
         OrgsTotal_Year = sum(OrgsTotal_Year)) %>%
  ungroup()
