...
years_ts_dir <- paste(report_year,'Plots/years_timeseries')
ry_ts_dir <- paste(report_year,'Plots/',report_year,'_timeseries')
ry_bar_dir <- paste(report_year,'Plots/',report_year,'_bargraphs')
...
df_month_sums <- df_benthic %>%
  # separate(Date, c('Month', 'Year'), ' ') #%>% # column already made
  group_by(Month, Year, StationCode) %>%
  mutate(Grab_Count = length(unique(Grab))) %>%
  ungroup() %>%
  ...
  summarize(
    Orgs_Total = sum(Count, na.rm = TRUE)
    ) %>%
  ungroup()

df_month_sums$Date <- with(df_month_sums, paste(Month, Year)) # explore adding in day as well

# get grab variable (TODO: maybe make more understandable)
...
df_year_sums <- df_month_sums %>%
  ...
  summarize(Orgs_Total = sum(Orgs_Total, na.rm = TRUE)) %>%
  ungroup()

df_year_sums <-  merge(df_year_sums, df_grab_vari, by = 'Year', all.x=FALSE, all.y=FALSE)

df_month_CPUE <- df_month_sums %>%
  group_by(.dots = c(classif, 'Month', 'Year')) %>% # TODO: redundant, check after confirming right
...
df_year <- df_year[!df_year$Phylum == 'n/a',] # TODO: need to fix in future versions of csv
...
# subset top 10 (B: 15) species by average CPUE

# TODO
# sum by year
# sum by all -- subset out top 15
# graph


df_year_top <- df_year %>%
  subset(Year >= report_year - 5) %>%
  group_by(Name) %>%
  mutate(Total_Mean = sum(CPUE_Total)) %>%
  arrange(desc(Total_Mean)) %>%
  ungroup()
  

df_year_top <- df_year_top[df_year_top$Total_Mean %in% unique(df_year_top$Total_Mean)[1:10],]

# complete cases
suppressWarnings(df_year_top <- df_year_top %>% complete(Year, nesting(Name, Total_Mean)))
df_year_top$CPUE_Total[is.na(df_year_top$CPUE_Total)] = 0
df_year_top$Total_Mean[is.na(df_year_top$Total_Mean)] = 0
...
# re-order
df_year_top$Name <- with(df_year_top, reorder(Name, Total_Mean))
...
df_month_stat_sums <- df_benthic %>%
  ...
  summarize(Orgs_Total = sum(Count, na.rm = TRUE)) %>%
  ungroup()
...
# factor month col
df_month_stat_top$Month = factor(df_month_stat_top$Month, levels = month.abb)
