...
df_values <- df_long %>%
    ...
    mutate(
        param = case_when(
            ...
            TRUE ~ col  # Default to column name
        )
    ) %>%
...
df_final <- bind_rows(df_head, df_blank, df_station, df_values, df_notes, df_latlong) %>%
  arrange(batch_id) %>%
  relocate(batch_id)
```

```{r}
write_csv(df_final, 'C:/Users/sperry/Desktop/SB_Jan25_draft.csv', na = '')
```
