...
abs_path_data <- function(fp_rel = NULL) {
  fp_emp <<- 'California Department of Water Resources/Environmental Monitoring Program - Documents/Water Quality/AQUARIUS Samples Database/Database Migration/2024'
...
```{r}
fp_file <- list.files(abs_path_data('Yearly WDL Data/'), full.names = TRUE, recursive = TRUE)
...
for (wq_file in fp_file){
  ...
  # export
  fp_exp <- abs_path_data(paste0('Formatted WDL Data/',gsub('.*/(\\d{4})\\.xlsx', '\\1', fn),'_formatted.csv'))
  
  readr::write_csv(df, fp_exp)
}
