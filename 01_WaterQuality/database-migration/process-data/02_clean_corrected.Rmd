  ...
  mutate(
    Depth =
      case_when(`Observed Property ID` %in% no_depths ~ NA_real_,
                ...
                grepl('\\(Bottom\\)', `Observed Property ID`) ~ as.numeric(`Water Depth Value`)-3,
                TRUE ~ 3)
    ) %>%
...
na_cols <- c('Observation ID','Activity ID','Collection Method','Field: Device ID','Lab: Limit Type','Lab: Received DateTime','Lab: Sample Fraction','EA_Data Review Flag','EA_Filter Container ID')
...
col_order <- c('Observation ID', 'Location ID', 'Observed Property ID', 'Observed DateTime', 'Analyzed DateTime', 
  ...
  'Lab: Received DateTime', 'Lab: Sample Fraction', 'Lab: From Laboratory', 
