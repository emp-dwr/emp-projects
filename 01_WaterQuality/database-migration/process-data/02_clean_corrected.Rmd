...
df_wide %>%
  filter(`Sample Type` == 'Blank; Field') %>%
  mutate(
    blank_check = ifelse(`Sample Type` == 'Blank; Field' & `Station Name` == lag(`Station Name`), TRUE, FALSE)
  ) %>%
...
cols_fixed <- c('Station Name', 'Station Number', 'Sample Date', 'Sample Time', 'Sample Code', 'Sample Depth', 'Sample Description', 'Sample Type', 'Parent Sample Code')
...
unique(df_long$original_colname[is.na(df_long$observed_property)])

# add modified column
df_long <- df_long %>%
  mutate('Lab: Modified Method_ex' = ifelse(`Sample Date` >= modified_first_date & 
                           `Sample Date` <= modified_last_date, 'TRUE', 'FALSE'))
```
...
```{r}
# pick random val when there's a lab duplicate
set.seed(42)
...
df_long <- df_long %>%
  mutate('Result Value' = sapply(`Result Value`, pick_val))
```
...
df_long <- df_long %>%
  rename(
    ...
    'Parent Activity Name_ex' = 'Parent Sample Code'
    ) %>%
  select(-c('Station Number'))

col_order <- c('Location ID','Observed Property ID','Observed DateTime','Depth','Depth Unit','Data Classification','Result Value','Result Unit','Activity Name','Parent Activity Name_ex','Field: Device Type','Sample Description','Lab: Analysis Method','Lab: Modified Method_ex', 'Lab: Detection Condition','Lab: MRL','Lab: From Laboratory','QC: Type')
...
write_csv(df_long, file = paste0(fp_export, '/prelim_data.csv'))
