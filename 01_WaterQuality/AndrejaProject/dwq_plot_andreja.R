...
func_clean_graph_dat <- function(df){
  ...
  df$FullGroup <- paste(df$Station,df$Monyear, sep = ' - ')
  ...
  df <-
    dplyr::select(c('Station','Month','Monyear','FullGroup','SpCndSurface_Sign','SpCndSurface_Val','TurbiditySurface_Sign','TurbiditySurface_Val','Chla_Sign','Chla_Val','DissAmmonia_Sign','DissAmmonia_Val','DissNitrateNitrite_Sign','DissNitrateNitrite_Val','TotPhos_Sign','TotPhos_Val'))
  
  # organize df
  df <- df %>%
    tidyr::pivot_longer(cols = contains('_'), names_pattern = '(.*)_(.*)', names_to = c('Analyte','.value'))
  
  df <- subset(df, !is.na(Station))
...
func_dwq_graphs <- function(){
  ...
  for (i in seq(length(analytes))){
    ...
    if (analytes[i] != 'SpCndSurface') {
      ...
      y_max_int <- max(df_dwq_filt[df_dwq_filt$Station %in% int_delta,]$Val, na.rm = TRUE)
      y_max_out <- max(df_dwq_filt[!df_dwq_filt$Station %in% int_delta,]$Val, na.rm = TRUE)
    }
    
    out <- by(data = df_dwq_filt, INDICES = df_dwq_filt$Station, FUN = function(m) {
      m <- droplevels(m)
      cur_region = as.character(unique(m$Station[[1]]))
      ...
      if (analytes[i] %in% unique(df_seg_vert$Analyte)){
        ...
        if(cur_region %in% unique(int_vert$Station)){
    ...
    ggplot2::ggsave(paste('01_WaterQuality/AndrejaProject/Graph_',analytes[i],'.jpg', sep=''), graph, width = 4.5, height = 5.3, unit = 'in')
  }
}
