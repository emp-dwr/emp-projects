...
library(zoo)
options(digits = 10)
source('03_Phyto/01_routine/phyco-cleaning/functions/phyco_funcs.R')

# read in DWQ run names and regions
df_names <- read_csv('03_Phyto/01_routine/phyco-cleaning/supp_files/run_names.csv', show_col_types = FALSE)

# obtain all archived MOPED file paths for specified year and create combo df
...
for(i in 1:nrow(df_combo)){
  ...
  df_wq <- df_wq %>% filter(Equipment == 'EXO Horizontal') #include only EXO Horizontal data
  #correct date time format for processing
  ...
  #create an Analyte column combining Header and Unit columns
  ...
  #convert df from long format to wide format
  df_wq_wide <- df_wq_replicates_rm %>%
...
df_wq_outliers_rm <- df_wq_outliers %>%
  ...
  df_wq_timeround <- df_wq_outliers_rm %>%
...
#  filter out rows around Antioch
  ...
  df_wq_antioch_rm <- df_wq_timeround %>%
  ...
  df_wq_antioch_rm$DateTime <- as.character(df_wq_antioch_rm$DateTime)
  
  # bind to df list
  df_list[[i]] <- df_wq_antioch_rm
