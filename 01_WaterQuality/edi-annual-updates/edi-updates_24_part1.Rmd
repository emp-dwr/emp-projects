...
unmapped <- df_wide %>%
  anti_join(df_stations, by = 'Station Name') %>%
  filter(!(`Station Name` %in% c('EZ2-SJR', 'EZ6-SJR'))) %>%
...
df_wide <- left_join(df_wide, df_stations, by = 'Station Name') %>%
  ...
  mutate(`Station Name` = case_when(
    `Station Name` == 'EZ2-SJR' ~ 'LSZ2-SJR',
    `Station Name` == 'EZ6-SJR' ~ 'LSZ6-SJR',
    TRUE ~ `Station Name`
  )) %>%
  select(-`New Station Name`)

stopifnot(!any(is.na(df_wide$`Station Name`) | !nzchar(df_wide$`Station Name`)))
...
stopifnot(!any(is.na(df_wide$`Sample Date`)))
```
...
rm(df_solids, df_compare, df_check)
```
...
pick_val <- function(cell) {
  cell <- as.character(cell)

  if (grepl('**', cell, fixed = TRUE) && grepl(',', cell)) {
    cell_vals <- trimws(unlist(strsplit(gsub('\\*\\*', '', cell), ',')))
    sample(cell_vals, 1)
  } else {
    cell
  }
...
df_long <- df_long %>%
  mutate(`Result Value` = vapply(`Result Value`, pick_val, character(1)))
```
...
df_analysis <- read_csv(abs_path_emp('Water Quality/AQUARIUS Samples Database/Database Migration/Lists to Import/Final Imports/Import_Analysis Methods.csv'), show_col_types = FALSE) %>%
...
```{r}
df_all <- df_all %>%
  rename(`EA_Field Quality Flag` = `EA_Data Review Flag`)
```
...
df_weather <- read_csv(abs_path_emp('Water Quality/AQUARIUS Samples Database/Data Projects/Weather Observations/AllWeather_Import.csv'), show_col_types = FALSE)

df_weather <- df_weather %>%
  mutate(
    `Observed DateTime` = gsub("\\[.*\\]", "", `Observed DateTime`),
    `Observed DateTime` = as.POSIXct(`Observed DateTime`, format = "%Y-%m-%d %H:%M:%S", tz = "")
  ) %>%
  mutate(Depth = as.character(Depth))

df_test <- df_all %>%
  mutate(`Observed DateTime` = `Observed DateTime` %>%
      gsub('\\[.*\\]', '', .) %>%  
      trimws() %>%
      ymd_hm()
  ) %>%
  filter(!(`QC: Type` %in% c('Replicate','Blank')))

weather_key_pairs <- df_weather %>%
  select(`Location ID`, `Observed DateTime`) %>%
  distinct() %>%
  mutate(`Observed Property ID` = "Weather Observations")

df_test <- df_test %>%
  mutate(`Observed DateTime` = format(`Observed DateTime`, "%Y-%m-%d %H:%M:%S"))

weather_key_pairs  <- weather_key_pairs  %>%
  mutate(`Observed DateTime` = format(`Observed DateTime`, "%Y-%m-%d %H:%M:%S"))
...
df_test_filt <- df_test %>%
  anti_join(weather_key_pairs, by = c("Location ID", "Observed Property ID", "Observed DateTime"))

df_final <- bind_rows(df_test_filt, df_weather)
```

ADD QC

```{r}
df_qc <- read_csv(abs_path_emp('Water Quality/AQUARIUS Samples Database/Data Projects/Field Quality Flags/Field_Quality_Flags_Import.csv'), show_col_types = FALSE)
...
df_test <- df_final %>%
  left_join(
    df_qc %>%
      select(`Location ID`, `Observed Property ID`, `Observed DateTime`, 
             EA_Field_Override = `EA_Field Quality Flag`),
    by = c("Location ID", "Observed Property ID", "Observed DateTime")
  ) %>%
  mutate(`EA_Field Quality Flag` = case_when(
    !is.na(EA_Field_Override) ~ EA_Field_Override,
    TRUE ~ `EA_Field Quality Flag` 
  )) %>%
  select(-EA_Field_Override) 
...
```{r}
# remove noncompliant TKN

# Remove TKN samples for lab noncompliance: 
# 	E0124B0109
# 	E0124B0190
# 	E0124B0339
# E0124B0340
...
df_test2 <- df_test %>%
  filter(!(`Observed Property ID` == 	'Total Kjeldahl Nitrogen' & `Activity Name` %in% c('E0124B0109','E0124B0190','E0124B0339','E0124B0340')))
...
```{r}
write_csv(df_edi, '01_WaterQuality/edi-annual-updates/data/edi_24_draft_throughjune.csv', na = '')
```
