...
df_edi <- read_csv('https://portal.edirepository.org/nis/dataviewer?packageid=edi.458.7&entityid=dfeaee030be901ae00b8c0449ea39e9c',
                   col_types = c(Time = 'c'),
                   show_col_type = FALSE)
df_wq <- read_csv('01_WaterQuality/edi-annual-updates/data/component/2022 WQ Data.csv', show_col_type = FALSE)
...
df_wq <- df_wq %>% dplyr::select(-REMOVE)
```
...
for (col in colnames(select_if(df_join, is.numeric))){
  check_graph(col, select_year = c(year-1,year))
}
