...
stations <- unique(df_stations$CDEC)
...
for (x in stations) {
  ...
  df_CWQ_all <- bind_rows(df_stations_all, df_temp)
...
# Add in known historical data for EMP sites not in CDEC
...
for (x in stations) {
  ...
  ggplot(df_temp,
    ...
    labs(x = NULL,
         y = NULL,
         #title = paste0("EMP Monitoring History at ",df_temp$ShortName)
         )
